...
class MixRec(nn.Module):
    ...
    def getAU(self, user, positive):
        ...
        align_loss = utility.losses.get_align_loss(user_embedding, item_embedding)
        uniform_loss = (utility.losses.get_uniform_loss(user_embedding) +
                        utility.losses.get_uniform_loss(item_embedding)) / 2
    ...
    def forward(self, user, positive, negative, epoch=None):
        ...
        bpr_loss = utility.losses.get_bpr_loss(user_embeddings, pos_embeddings, neg_embeddings)
        bpr_loss_2 = utility.losses.get_InfoNCE_loss_all(user_embeddings, pos_embeddings, mix_neg_embeddings_2, 1.0)
        ...
        user_loss = utility.losses.get_InfoNCE_loss_all(user_embeddings, cl_user_embeddings, cl_user_embeddings_2, self.temperature)
        user_loss_2 = utility.losses.get_InfoNCE_loss_all(user_embeddings_2, cl_user_embeddings, cl_user_embeddings_3, self.temperature)
        user_ssl_loss = user_beta * user_loss + (1 - user_beta) * user_loss_2

        item_loss = utility.losses.get_InfoNCE_loss_all(pos_embeddings, cl_item_embeddings, cl_item_embeddings_2, self.temperature)
        item_loss_2 = utility.losses.get_InfoNCE_loss_all(pos_embeddings_2, cl_item_embeddings, cl_item_embeddings_3, self.temperature)
